...
// Helper function to parse price from string like "₹379" or "₹ 429" or "₹ 1,300"
const parsePrice = (priceStr: string): number => {
  return parseInt(priceStr.replace(/[₹,\s]/g, ''), 10);
};

// Helper function to determine category
const getCategory = (name: string, section: string): string => {
  const lowerName = name.toLowerCase();
  if (section.includes('VEG') || lowerName.includes('veg') || lowerName.includes('paneer') || lowerName.includes('dal') || lowerName.includes('aloo') || lowerName.includes('gobi') || lowerName.includes('mushroom') || lowerName.includes('mix veg') || lowerName.includes('subz')) {
    return 'veg';
  }
  if (lowerName.includes('chicken') || lowerName.includes('fish') || lowerName.includes('prawn') || lowerName.includes('mutton') || lowerName.includes('keema') || lowerName.includes('egg') || lowerName.includes('kodi') || lowerName.includes('murg') || lowerName.includes('tangdi') || lowerName.includes('kabab') || lowerName.includes('kebab') || lowerName.includes('lamb') || lowerName.includes('harissa') || lowerName.includes('mahi')) {
    return 'non-veg';
  }
...
const specialMenu128 = {
  drinks: [
    { name: 'Dewars White Label', price: 128 },
    { name: 'Smirnoff Vodka', price: 128 },
    { name: 'Bacardi Rum (White/Black/Mango Chilly/Ginger/Orange/Lemon)', price: 128 },
    { name: 'Old Monk Legend Rum', price: 128 },
    { name: 'Kyron Brandy', price: 128 },
    { name: 'Great Indian Gin', price: 128 },
    { name: 'KF Premium Draught Beer (330ml)', price: null },
    { name: 'DesmondJi Tequila 51% Agave', price: 128 },
  ],
    cocktails: [
    { name: 'Whiskey Sour', price: 128 },
    { name: 'Caipiroska', price: 128 },
    { name: 'Cosmopolitan', price: 128 },
    { name: 'Mojito', price: 128 },
    { name: 'Daiquiri', price: 128 },
    { name: 'Gimlet', price: 128 },
    ],
    mocktails: [
    { name: 'Nojito', price: 128 },
    { name: 'Blue Angel', price: 128 },
    { name: 'Strawberry Colada', price: 128 },
    { name: 'Virgin Guava Marry', price: 128 },
    { name: 'Mango Bloom', price: 128 },
    { name: 'Cold Pressed Juices', price: 128 },
  ],
  kfBeerPrices: [
    { name: 'KF Ultra', price: 189 },
    { name: 'KF Ultra Max', price: 199 },
    { name: 'KF Ultra Wit', price: 269 },
  ],
  food: {
    nonVeg: [
      { name: 'Murgh Tikka (3pcs)', price: 128 },
      { name: 'Murgh Seekh Kebab (1pc)', price: 128 },
      { name: 'Kodi Vepudu', price: 128 },
      { name: 'Mirayala Kodi Vepudu', price: 128 },
      { name: 'Chicken Ghee Roast', price: 128 },
      { name: 'Chicken Pakoda', price: 128 },
      { name: 'Dhaba Fried Chicken', price: 128 },
      { name: 'Pepper Chicken', price: 128 },
      { name: 'Chicken 65', price: 128 },
      { name: 'Chicken Shanghai Rolls', price: 128 },
      { name: 'Chicken Nuggets', price: 128 },
      { name: 'Chicken Popcorn', price: 128 },
      { name: 'Crispy Fried Chicken (3pcs)', price: 128 },
      { name: 'Peri Peri Chicken Wings (4pcs)', price: 128 },
      { name: 'Crispy Fried Wings (4pcs)', price: 128 },
      { name: 'Thai Pai Chicken', price: 128 },
      { name: 'Thai Chilli Fish', price: 128 },
      { name: 'Apollo Fish', price: 128 },
      { name: 'Crispy Fried Fish (3pcs)', price: 128 },
    ],
    vegAndEgg: [
      { name: 'Peanut Masala (Boiled/Fry)', price: 128 },
      { name: 'Onion Rings (5pcs)', price: 128 },
      { name: 'Veg Nuggets', price: 128 },
      { name: 'Green Salad', price: 128 },
      { name: 'Cheese Garlic Bread (3pcs)', price: 128 },
      { name: 'Crispy Corn', price: 128 },
      { name: 'Corn Tikki (4pcs)', price: 128 },
      { name: 'Corn Cheese Bites (4pcs)', price: 128 },
      { name: 'French Fries (Salted / Peri Peri)', price: 128 },
      { name: 'Potato Wedges', price: 128 },
      { name: 'Hara Bhara Kebab (4pcs)', price: 128 },
      { name: 'Tandoori Gobi (4pcs)', price: 128 },
      { name: 'Paneer Tikka (3pcs)', price: 128 },
      { name: 'Manchurian (Veg/Gobi)', price: 128 },
      { name: '65 (Gobi/Paneer/Aloo/Egg)', price: 128 },
      { name: 'Chilli (Paneer/Egg)', price: 128 },
    ],
  },
};

const specialCategoryConfig = [
  { key: 'drinks', label: 'Premium Drinks', icon: '🥃' },
  { key: 'cocktails', label: 'Cocktails', icon: '🍸' },
  { key: 'mocktails', label: 'Mocktails', icon: '🥤' },
  { key: 'kfBeer', label: 'KF Beer', icon: '🍺' },
  { key: 'nonVeg', label: 'Non-Veg Bites', icon: '🍗' },
  { key: 'vegAndEgg', label: 'Veg & Egg Bites', icon: '🥗' },
] as const;
          ...
          >
            <div className="bg-gradient-to-r from-[#2563EB]/20 to-[#3B82F6]/20 backdrop-blur-sm rounded-2xl p-4 border border-[#2563EB]/30 max-w-2xl">
              <p className="text-white font-semibold text-lg mb-2">
                🎉 Eat and Drink Anything @ ₹128 from 12PM - 8PM
              </p>
          ...
          {Object.keys(menuData).map((section) => (
            <motion.button
              ...
              onClick={() => {
                ...
                setSelectedCategories([]); // Reset filters when switching sections
